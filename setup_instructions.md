...
1. Download & install Anaconda for Python 3.8 [here](https://www.anaconda.com/distribution/)
...
```
git clone https://github.com/davidanastasiu/coen281-wi21.git
```

Now you should have all the materials in a `coen281-wi21` folder; to get the latest versions, just run:
...
### Running stuff
In the directory where the relevant course materials are (if you followed the above step, this will be in the `coen281-wi21` directory), run:
...
### Python 3.8:
Ideally you are using this version- make sure you are not using Python 2!
...
For example, if you successfully installed jupyter but it's complaining that the command is not found, try adding `~/Library/Python/3.XXX/bin` (for Mac) and `~/bin` (for Linux) to your path, where XXX = your version of python (probably 3.7). I.e., on Mac OS / linux, edit your `~/.bash_profile` or `~/.bashrc` file and add the line, replacing XXX as appropriate:
...
<sup>Instal notes adapted with permission from ones by Chris R̩.</sub>
